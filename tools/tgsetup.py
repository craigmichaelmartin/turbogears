...
# TGBEGIN
win32 = """
The recommended Python for TurboGears 1.0 is Python 2.5.2.
...
def check_subprocess():
    # specific check for python 2.3 on Windows
    major, minor, minor2, tag, sub = sys.version_info
    if sys.platform == "win32" and major == 2 and minor == 3:
        try:
            import _subprocess

        except ImportError:
            print """Windows subprocess module must be installed, please download and install:
            http://effbot.org/downloads/subprocess-0.1-20041012.win32-py2.3.exe
            """
            # get out we cannot continue
            sys.exit(1)

def check_python():
    if sys.version_info < (2, 3):
        print "Python 2.3 or higher is required to run TurboGears 1.0."

        if not get_python():
            sys.exit(1)

    elif sys.version_info < (2, 4):
        print "TurboGears 1.0 supports Python 2.3, but 2.4 is strongly recommended."

        if not get_python():
            print "Would you like to continue with a Python 2.3 install?yes/no[n]"
        ...
        print "Python 2.6 or higher is not supported by TurboGears 1.0."
