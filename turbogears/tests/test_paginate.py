...
query_methods = 'Q QA SR SO SL'.split()
if SASelectResults is None:
    query_methods.remove('SR')
...
class TestBasicPagination(TestPagination):
    ...
    class MyRoot(RootController):
        ...
        def basic(self):
            spy = Spy(var_name='data', pages=xrange(1, 4), limit=4, page_count=3,
        ...
        def dynamic(self):
            spy = Spy(var_name='data', pages=xrange(1, 3), limit=7, page_count=2,
                      order=None, ordering=[], row_count=10)
            data = range(10)
        ...
        def multiple(self):
            spy_foo = Spy(name='foo', var_name='foo', pages=xrange(1, 6),
                          limit=4, page_count=5, order=None, ordering=[],
                          row_count=20)
            spy_bar = Spy(name='bar', var_name='bar', pages=xrange(1, 4), limit=10,
    ...
    def test_strict_parameters(self):
        ...
        try:
            ...
            assert '"data": [0, 1]' in self.body
            Spy.assert_ok(self.body, 'limit', 2)
            self.request("/default_max_pages?tg_paginate_no=5")
            assert self.status.startswith('200')
            assert '"data": [40, 41, 42, ' in self.body
            Spy.assert_ok(self.body, 'pages', xrange(3, 8))
            self.request("/basic?tg_paginate_rubbish=2")
            assert self.status.startswith('500')
            assert ("basic() got an unexpected keyword argument"
                " 'tg_paginate_rubbish'") in self.body
            # new style
            self.request("/basic?data_tgp_no=2")
            assert self.status.startswith('200')
            assert '"data": [4, 5, 6, 7]' in self.body
            ...
            assert '"data": [40, 41, 42, ' in self.body
            Spy.assert_ok(self.body, 'pages', xrange(3, 8))
            self.request("/basic?data_tgp_rubbish=2")
            assert self.status.startswith('500')
            assert ("basic() got an unexpected keyword argument"
    ...
    def test_with_webpath(self):
        ...
        try:
            self.request("/web/path/basic?data_tgp_no=2")
            Spy.assert_ok(self.body, 'current_page', 2)
...
class TestDatabasePagination(TestPagination):
    ...
    def test_related_objects_ordering_level_1(self):
        # +/+
        self.request("/related?tg_paginate_ordering=%s" % quote('user.name,id'))
