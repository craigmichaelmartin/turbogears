...
except ImportError: # formencode version < 1.1
    national = util.Bunch()
    try:
        national.update(USStateProvince=StateProvince,
            USPhoneNumber=PhoneNumber,
            USPostalCode=PostalCode)
    except NameError: # recursive import problem
        pass
    try:
        national.update(dict(
            InternationalPhoneNumber=IPhoneNumberValidator))
    except NameError: # formencode version < 0.9
        pass
else:
    StateProvince = national.USStateProvince
    PhoneNumber = national.USPhoneNumber
    PostalCode = national.USPostalCode
    IPhoneNumberValidator = national.InternationalPhoneNumber
